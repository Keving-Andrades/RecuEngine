...
const activeForm = formViews => {
	...
	let activeForm;
	forms.forEach(e => {
		if (e.classList.contains('front') === true) {
			activeForm = e;
		};
	});
...
const nextForm = (index, formViews, formNavbar, nextBtn) => {
	...
	setTimeout(() => {
		...
		if ((isFormAutofilled(forms[index].querySelectorAll('input')) === true || forms[index].classList.contains('valid') === true) && nextBtn.innerText === 'Siguiente') {
			nextBtn.classList.add('next');
		} else {
			nextBtn.classList.remove('next');
		};
		...
		form.addEventListener('form', event => {
			if (index + 1 < forms.length) {
				if (event.detail.valid === true) {
					activateNextForm(index, formViews, formNavbar, nextBtn);
				} else {
					deactivateNextForm(index, formNavbar, nextBtn);
				};
			};
		...
		if (index > 0) {
			...
			formInputs.forEach(input => {
				input.addEventListener('keyup', async () => {
					if (input.checkValidity() === true && input.classList.contains('invalid')) input.classList.remove('invalid');
...
const nextBtnClickListener = (nextBtn, formViews, formNavbar, registerForm) => {
	...
	nextBtn.addEventListener('click', () => {
		if (nextBtn.innerText === 'Enviar' && nextBtn.classList.contains('submit') && registerForm.checkValidity() === true) {
			registerForm.submit();
		};
		...
		if (nextBtn.innerText === 'Siguiente' && activeForm(formViews) && activeForm(formViews).classList.contains('ready') && nextBtn.classList.contains('next')) {
			...
			if (activeForm(formViews) && !activeForm(formViews).classList.contains('valid')) {
				if (activeForm(formViews) === forms[0]) {
					...
					if (Array.from(activeForm(formViews).querySelectorAll('input')).find(input => input.value.length === 0 || input.checkValidity() === false).value.length === 0) {
						Array.from(activeForm(formViews).querySelectorAll('input')).find(input => input.value.length === 0 || input.checkValidity() === false).focus();
					} else {
						Array.from(activeForm(formViews).querySelectorAll('input')).find(input => input.value.length === 0 || input.checkValidity() === false).classList.add('invalid');
						Array.from(activeForm(formViews).querySelectorAll('input')).find(input => input.value.length === 0 || input.checkValidity() === false).focus();
					}
				};
			};
...
/**
 * The constructor function of the "Registration" class takes as arguments multiple HTML elements of the registration form.
 * @class
